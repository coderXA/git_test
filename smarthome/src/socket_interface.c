...
static void* tcpsocket_get(void *arg)
{
	...
	while (1)
	{
		...
		ret = setsockopt(c_fd, SOL_SOCKET, SO_KEEPALIVE, &keepalive,\
		sizeof(keepalive)); // 设置TCP_KEEPALIVE选项
		if (ret == -1) { // 如果设置失败，打印错误信息并跳出循环
			perror("setsockopt");
			break;
		}
		ret = setsockopt(c_fd, IPPROTO_TCP, TCP_KEEPIDLE, &keepidle,\
		sizeof(keepidle)); // 设置探测时间间隔选项
		if (ret == -1) { // 如果设置失败，打印错误信息并跳出循环
			perror("setsockopt");
			break;
		}
