...
static void* handle_device(void *arg)
{
	...
	if (cur_gdev != NULL)
	{
		...
		{
			memset(oled_msg, 0, sizeof(oled_msg));
			strcpy(oled_msg, "A risk or fire!\n");
		}
		printf("oled_msg=%s\n",oled_msg);
		oled_show(oled_msg);

		//special for lock, close lock
		if (1 == cur_gdev->check_face_status && face_result > 0.6 && ret == 0)
		{
			sleep(5);
...
static void* receive_get(void *arg)
{
	...
	if (NULL != arg)
	{
		...
		recv_msg->ctrl_info = (ctrl_info_t *)arg;//获取到mqd和phead(struct control 链表的头结点)
		recv_msg->msg_len = -1;
